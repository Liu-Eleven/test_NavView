...
QSize NavDelegate::sizeHint( const QStyleOptionViewItem &option, const QModelIndex &index ) const
{
 ...
 void NavDelegate::setHoverRow(int row) {
     hoverRow = row;
 }
...
//    //else if ( option.state & QStyle::State_MouseOver )//mouse over
//	{
...
//	}
    ...
    if ( index.row() == hoverRow )//hoverRow
       {
           painter->fillRect(option.rect, color_hover);
       }
	...
	if ( node->children.size() != 0 )
	{
		...
		{
			if ( option.state & QStyle::State_Selected )
				imgPath = ":/Image/unexpand_selected.png";
			else
			    imgPath = ":/Image/unexpand_normal.png";
		}
		else
		{
			if ( option.state & QStyle::State_Selected )
				imgPath = ":/Image/expand_selected.png";
			else
				imgPath = ":/Image/expand_normal.png";
		}
